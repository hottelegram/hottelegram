...
def main():
    ...
    # Criptografando o conteúdo original do body
    encrypted_body = xor_encrypt(original_body_content, xor_key)
    # Convertendo para base64 para evitar problemas de codificação no HTML
    b64_body = base64.b64encode(encrypted_body.encode('utf-8')).decode('utf-8')

    # Script JavaScript de segurança a ser injetado
    # Este script fará:
    # 1. Verificação de Domínio (Domain Lock)
    # 2. Descriptografia e injeção do conteúdo real
    # 3. Bloqueio de clique direito, atalhos, arrastar, seleção de texto
    # 4. Anti-DevTools avançado com detecção e debugger loop
    ...
    (function() {{
        ...
        const key = "{xor_key}";
        const authorizedDomains = ["hottelegram.github.io", "localhost", "127.0.0.1", ""]; // Vazio permite visualização via file://
        ...
        // Se o protocolo for file://, permite para testes
    ...
    # 1. CSS anti-seleção para injetar no head
    anti_select_css = """
    ...
    *, html, body {
    ...
    # O script de descriptografia roda imediatamente após a montagem do DOM.
    loader_html = """
<body>
...
"""
    # Substituir o loader pelo código do JS de segurança de forma limpa
    ...
    print(f"Escrevendo arquivo final protegido em {prod_path}...")
    with open(prod_path, "w", encoding="utf-8") as f:
        f.write(final_html)

    print("Protecao aplicada com sucesso!")
